...
FramelessWidgetLinux::FramelessWidgetLinux(bool p_frameless, QWidget *p_parent)
...
bool FramelessWidgetLinux::eventFilter(QObject *p_obj, QEvent *p_event)
{
    if (!m_frameless) {
        return FramelessWidget::eventFilter(p_obj, p_event);
    }
...
void FramelessWidgetLinux::setTitleBar(QWidget *p_titleBar)
{
    FramelessWidget::setTitleBar(p_titleBar);

    m_titleBar->installEventFilter(this);
}

void FramelessWidgetLinux::mousePressEvent(QMouseEvent *p_event)
{
    FramelessWidget::mousePressEvent(p_event);
...
void FramelessWidgetLinux::recordMousePress(const QPoint &p_pos)
{
    m_mousePressed = true;
...
void FramelessWidgetLinux::mouseReleaseEvent(QMouseEvent *p_event)
{
...
void FramelessWidgetLinux::mouseMoveEvent(QMouseEvent *p_event)
{
    FramelessWidget::mouseMoveEvent(p_event);
...
void FramelessWidgetLinux::doResize(QEvent *p_event)
{
    Q_UNUSED(p_event);
#if 0
...
FramelessWidgetLinux::Area FramelessWidgetLinux::hitArea(const QPoint &p_pos) const
{
    const int x = p_pos.x();
    const int y = p_pos.y();
    Area area = Area::None;
...
void FramelessWidgetLinux::showEvent(QShowEvent *p_event)
{
    FramelessWidget::showEvent(p_event);
