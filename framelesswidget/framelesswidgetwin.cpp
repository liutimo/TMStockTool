...
FramelessWidgetWin::FramelessWidgetWin(bool p_frameless, QWidget *p_parent)
    : FramelessWidget(p_frameless, p_parent)
{
    if (m_frameless) {
        ...
        connect(m_redrawTimer, &QTimer::timeout,
                this, &FramelessWidgetWin::forceRedraw);

        connect(this, &FramelessWidget::windowStateChanged,
                this, &FramelessWidgetWin::updateMargins);
...
bool FramelessWidgetWin::nativeEvent(const QByteArray &p_eventType, void *p_message, qintptr *p_result)
#else
bool FramelessWidgetWin::nativeEvent(const QByteArray &p_eventType, void *p_message, long *p_result)
#endif
{
    if (!m_frameless) {
        return FramelessWidget::nativeEvent(p_eventType, p_message, p_result);
    }
...
void FramelessWidgetWin::moveEvent(QMoveEvent *p_event)
{
    FramelessWidget::moveEvent(p_event);
...
void FramelessWidgetWin::updateMargins()
{
    if (!m_frameless) {
        return;
    }
...
void FramelessWidgetWin::forceRedraw()
{
...
void FramelessWidgetWin::setWindowFlagsOnUpdate()
{
    if (m_frameless) {
